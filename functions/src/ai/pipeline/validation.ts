...
import { STYLE_PRIMARY, WARNINGS } from '../../models/contracts';
...
export function validateAnalysisResult(result: any): ValidationResult {
  ...
  if (!result) {
	...
	} else if (!STYLE_PRIMARY.includes(result.style_primary.value)) {
		errors.push(`Invalid classification: ${result.style_primary.value}`);
	}

  // Validate substyle if present
	if (result.substyle && result.substyle.value && result.style_primary?.value) {
		if (!isValidSubtype(result.style_primary.value, result.substyle.value)) {
  ...
  if (result.style_primary && (!result.style_primary.evidence_keys || !Array.isArray(result.style_primary.evidence_keys))) {
...
export function applySanityRules(parsedData: any, analysisResult: any): ValidationResult {
  ...
  if (analysisResult.use_cases?.some((uc: any) => uc.value === 'ui')) {
    if (analysisResult.style_primary?.value === 'serif' && !analysisResult.style_primary.evidence_keys?.some((e: string) => e.includes('serifDetected=false'))) {
