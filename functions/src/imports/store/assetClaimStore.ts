...
export async function claimAsset(
  ...
  return db.runTransaction(async (tx) => {
    ...
    const expires = current?.leaseExpiresAt instanceof Date ? current.leaseExpiresAt : undefined;
...
export async function commitAssetClaim(
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (current.claimId !== claimId(input) || !(current.leaseExpiresAt instanceof Date) || current.leaseExpiresAt <= now) return { kind: "not_claimed" };
