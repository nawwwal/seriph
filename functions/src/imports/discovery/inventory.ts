...
import { classifyRole, type ClassifiedInventory } from "./classifyRole";
...
async function consume(source: ByteSource): Promise<{ prefix: Uint8Array; sha256: string; byteSize: number }> {
  ...
  return { prefix: head, sha256: hash.digest("hex"), byteSize };
}
...
export async function buildInventoryItem(input: InventoryInput): Promise<InventoryItem> {
  ...
  const classified = classifyRole({ bytes: consumed.prefix, name: input.name ?? input.filename });
