...
interface FamilyCoverProps {
  ...
  mode: 'spines' | 'covers';
  /** Reserved for future handcrafted covers. */
  coverSeed?: number;
  description?: string;
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-1 flex items-end p-4 sm:p-5 md:p-6 bg-[color-mix(in_srgb,var(--ink)_6%,var(--paper))]">
        <div
          className="w-full relative z-10 text-6xl sm:text-7xl lg:text-6xl xl:text-7xl leading-none font-black uppercase tracking-normal family-sample truncate-2"
          style={{ fontFamily: family.name, letterSpacing: '0' }}
        >
          {sampleChars}
        </div>
      </div>
      <div className="rule-t p-3 sm:p-4 bg-[var(--paper)]">
        <div className="text-xl font-extrabold truncate family-name" style={{ fontFamily: family.name }}>
          {family.name}
        </div>
        {description && (
        ...
        <div className="mt-3 flex justify-between items-center text-xs uppercase">
          ...
          <div className="flex items-center gap-1.5">
            {isVariable && (
              <span className="font-bold text-[10px] rule px-1.5 py-0.5 rounded-[var(--radius)]">Var</span>
            )}
