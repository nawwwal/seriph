...
export function useImportBatchFeed({ user, isAuthLoading, onCompletion, listener, api }: { user: User | null; isAuthLoading: boolean; onCompletion?: (event: FamiliesAppliedEvent) => void; listener?: BatchFeedListener; api?: BatchFeedApi }) {
  const [feed, setFeed] = useState<BatchFeedState>({ batches: [], activeCount: 0, transport: 'realtime', nextCursor: null });
  ...
  useEffect(() => {
    if (isAuthLoading || !user?.uid) { setFeed({ batches: [], activeCount: 0, transport: 'realtime', nextCursor: null }); return; }
    const next = createBatchFeedController({ listener: listener ?? createFirestoreBatchFeedListener(user.uid), api: api ?? createImportBatchApi(user), onChange: setFeed, onCompletion }); controller.current = next; next.start();
