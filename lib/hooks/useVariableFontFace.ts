...
export function useVariableFontFace(
  ...
  useEffect(() => {
    ...
    const cssFormat = ext === 'woff2' ? 'woff2' : format === 'TTF' ? 'truetype' : String(format).toLowerCase();
    // Single-face registration so font-variation-settings is not defeated by static face matching.
