...
class Tx { wrote = false; sourceWrites = 0; constructor(private db: Db) {} async get(ref: Ref) { if (this.wrote) throw new Error('read after write'); return ref.get(); } set(ref: Ref, data: Data) { this.wrote = true; if (ref.path.includes('/sources/')) this.sourceWrites++; this.db.docs.set(ref.path, data); } update(ref: Ref, data: Data) { this.wrote = true; if (ref.path.includes('/sources/')) this.sourceWrites++; this.db.docs.set(ref.path, { ...this.db.docs.get(ref.path), ...data }); } }
class Db { docs = new Map<string, Data>(); sourceWrites: number[] = []; collection(name: string) { return { doc: (id: string) => new Ref(`${name}/${id}`, this) }; } runTransaction<T>(run: (tx: Tx) => Promise<T>) { const tx = new Tx(this); return run(tx).then((result) => { this.sourceWrites.push(tx.sourceWrites); return result; }); } }
...
describe('import source commands', () => {
  ...
  it('chunks over-cap inventory into transactions with at most two hundred source writes', async () => {
    ...
    expect(result).toHaveLength(201); expect(result[200]).toMatchObject({ accepted: false, state: 'failed', errorCode: 'request_source_limit' }); expect(db.sourceWrites).toEqual([200, 1]);
  });

  it('rejects Windows drive-root paths while retaining both path forms', async () => {
    const db = new Db(); const input = { ...source, relativePath: 'C:\\private\\Font.zip' }; const result = sources(await registerImportSources(firestore(db), batch(db, 1), [input]));
    expect(result[0]).toMatchObject({ accepted: false, errorCode: 'invalid_path' }); expect(db.docs.get('users/ada/importBatches/b1/sources/' + source.sourceId)).toMatchObject({ relativePath: input.relativePath, normalizedRelativePath: 'C:/private/Font.zip' });
  });

  it('rejects non-canonical and repeated IDs without creating duplicate source counters', async () => {
    const db = new Db(); const input = batch(db, 3); const result = sources(await registerImportSources(firestore(db), input, [source, source, { ...source, sourceId: 'not/a/uuid' }]));
