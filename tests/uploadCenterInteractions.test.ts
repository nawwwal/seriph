...
const state = vi.hoisted(() => ({ batches: [] as ImportBatchSummary[], close: vi.fn(), isOpen: true, loadChildren: vi.fn(), open: vi.fn(), sourceProgress: {} }));
...
describe('Upload Center rendered interactions', () => {
  it('clicking a filter control changes the displayed batch rows', async () => {
    state.batches = [makeBatch('Active archive', 'active'), makeBatch('Review archive', 'needs_review')];
  ...
  it('calculates the current phase from the canonical batch contract', async () => {
