...
describe('family detail client loader', () => {
  ...
  it('dedupes concurrent requests for the same user and family', async () => {
    const fetchMock = mockFamilyFetch(); vi.stubGlobal('fetch', fetchMock);
    const getIdToken = vi.fn(async () => 'token'); const input = { uid: 'user-a', familyId: 'inter', getIdToken };

    const first = loadFamilyDetail(input); const second = loadFamilyDetail(input);

    expect(first).toBe(second);
    await expect(first).resolves.toMatchObject({ kind: 'loaded', source: 'network', family: { id: 'inter', name: 'Inter' } });
    expect(fetchMock).toHaveBeenCalledTimes(1); expect(getIdToken).toHaveBeenCalledTimes(1); expect(getCachedFamily('user-a', 'inter')?.name).toBe('Inter');
  });

  it('serves cached detail without another network request', async () => {
    const fetchMock = mockFamilyFetch(); vi.stubGlobal('fetch', fetchMock);
    await loadFamilyDetail({ uid: 'user-a', familyId: 'inter', getIdToken: async () => 'token' }); fetchMock.mockClear();

    await expect(loadFamilyDetail({ uid: 'user-a', familyId: 'inter', getIdToken: async () => 'token' })).resolves
      .toMatchObject({ kind: 'loaded', source: 'memory', family: { id: 'inter' } });
  ...
  it('hydrates an account-scoped persisted detail before making a network request', async () => {
    ...
    await expect(loadFamilyDetail({ uid: 'user-a', familyId: 'inter', getIdToken: async () => 'token' })).resolves
      .toMatchObject({ kind: 'loaded', source: 'snapshot', family: { id: 'inter' } });
    expect(fetchMock).not.toHaveBeenCalled();
  });
  ...
  it('caches returned details under the requested route id when it differs from the canonical id', async () => {
    ...
    expect(getCachedFamily('user-a', 'inter')?.id).toBe('canonical-inter'); expect(getCachedFamily('user-a', 'canonical-inter')?.id).toBe('canonical-inter');
  ...
  it('normalizes missing dates and clones font arrays', () => {
    ...
    expect(serialized?.uploadDate).toBe(''); expect(serialized?.fonts).toEqual(rawFamily.fonts);
